...
public class SQL5 {
	...
	public SQL5(int salesOrProduction, int productId, int companyId,int fromYear,int toYear,int swap, String incExCountries,
		...
		if (swap==1) {
	      ...
	      query = " select a.year, a.quantity,  CASE  WHEN substr(b.name,1,20) = '_OTHER' then '  OTHER' ELSE substr(b.name,1,20) END  as company, d.name as product, "+country+" as country " +
